...
- **ADS-family 403s are now self-diagnosing.** The upstream Server Groups v2 search endpoints return a bare `403` for global/account-scoped keys that is indistinguishable from an RBAC denial, so callers (and models) chase permissions instead of the actual fix. The search/saved-search/assignments workflows now append a hint to any 403 — it's typically the global-key signature; re-issue the key org-scoped (zone Settings > Secrets & Keys) — at the exact failure point, with no configuration to declare or drift. The metadata endpoints (`get_searchable_fields`, `get_search_scopes`, `get_device_metadata_fields`) are deliberately excluded: they accept either key type, so a 403 there means something else. (A declared `KEY_SCOPE` env var was considered and rejected — users who don't know their key type would set it wrong, and it drifts on key rotation.)
