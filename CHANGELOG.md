...
- **`list_events` `count_only` now actually returns a count.** The parameter was sent as snake_case `count_only=true` — an unknown query param the `/events` API silently ignores — so a "count only" call returned a normal limit-capped page (up to 500 event bodies) and `total_events` reported the page size, not the real total. The API's count switch is camelCase `countOnly` with the **integer** `1` (its upstream validator is a Laravel boolean rule that accepts `1`/`0` but 400s on the strings `"true"`/`"false"` with a misleading "must be true or false" message); it responds `{"size": <total>, "results": []}` and honors every other filter (`event_name`, date range, etc.). The workflow now sends `countOnly=1` and reads `size`, so `count_only=True` returns the true matching-event total with an empty `events` list — a single cheap call instead of paginating. The response parser was corrected to the **real** shapes (a bare list for a normal query; `{size, results}` in count mode) — it previously looked for a `{"data": [...], "total": N}` shape the endpoint never returns — and the unit test was rebased onto a captured count payload (it had asserted that same fabricated shape, so it passed while the feature did nothing). Verified against a live tenant (`tests/probe_events_count.py`).

### Security

- **Bumped three dependencies flagged by the scheduled dependency audit** (`pip-audit` over the exported lockfile). `cryptography` 48.0.0 → 49.0.0 (GHSA-537c-gmf6-5ccf — the OpenSSL statically linked into PyPI wheels before 48.0.1 was vulnerable), `pydantic-settings` 2.13.1 → 2.14.2 (GHSA-4xgf-cpjx-pc3j — `NestedSecretsSettingsSource` followed symlinks resolving outside `secrets_dir` and bypassed the `secrets_dir_max_size` cap), and `starlette` 1.2.1 → 1.3.1 (CVE-2026-54283 — `max_fields`/`max_part_size` were silently ignored for `application/x-www-form-urlencoded`, a DoS; CVE-2026-54282 — an unvalidated request path could cross into the URL authority and poison `request.url.hostname`). Security floors were added/raised in `[tool.uv] constraint-dependencies` (`pydantic-settings>=2.14.2` added; `starlette` `>=1.0.1` → `>=1.3.1`; `cryptography` `>=46.0.6` → `>=48.0.1`) and the direct `cryptography` floor in `[project].dependencies` was raised to `>=48.0.1`, so a fresh resolve cannot regress into the vulnerable ranges. `pip-audit` over the regenerated lock reports no known vulnerabilities.
