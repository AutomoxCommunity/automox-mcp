...
- **`AUTOMOX_MCP_ALLOW_SPLASHTOP_BULK_INSTALL_UNINSTALL` env gate.** New default-off flag controlling the fleet-scale `splashtop_bulk_install_uninstall` tool, parallel to `AUTOMOX_MCP_ALLOW_APPLY_REMEDIATION_ACTIONS`. The flag name mirrors the tool so it reads as "gate deploying/removing the client software fleet-wide," not "gate starting remote-control sessions" (that is `splashtop_initiate_connection`, which is not env-gated).
