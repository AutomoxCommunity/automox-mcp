...
The server exposes 99 tools across devices, policies, patches, groups, webhooks, worklets, vulnerability sync, maintenance windows, and more. You don't need to know the tool names — just describe what you want:
...
| `AUTOMOX_MCP_READ_ONLY` | No | `false` | Disable all write operations (67 of 99 tools remain) |
...
Disables all write operations. Only read-only tools are registered (67 of 99). Useful for auditing and monitoring.
...
- **MCP Tool Annotations** on all 99 tools — `readOnlyHint`, `destructiveHint`, `idempotentHint`, and `openWorldHint` per the MCP Protocol specification, enabling client-side confirmation dialogs and safety guardrails
