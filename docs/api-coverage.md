...
The access-certification (RBAC) review App (`ui://automox/access-certification.html`, on `list_users`) is deliberately **read-first**: no gated change is wired from it because there is no role/permission-assignment write tool and `list_users` projects a numeric `id`, not the UUID `remove_user_from_account` requires. Wiring a gated change from that view (a role-assignment write tool and/or projecting the user UUID) is net-new work, tracked as a follow-up — not a reuse of an existing tool.
