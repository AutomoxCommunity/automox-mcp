...
- Enable **confirmation dialogs** in your MCP client for all write tools (the server's 43 write tools are identifiable by their MCP Tool Annotation `readOnlyHint: false`; the 42 that mutate state also carry `destructiveHint: true`)
