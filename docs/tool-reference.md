...
- **`search_devices`** - Search Automox devices by hostname, IP, tag, status, or severity of missing patches. Supports multi-severity filtering (e.g., `["critical", "high"]`); the requested severities are echoed in `metadata.filters.severity` and every returned device has at least one matching patch. Per-device `pending_patches` is the device's total outstanding-patch count, not the severity-filtered subset.
