...
_DOMAIN_CATALOG: dict[str, list[tuple[str, str]]] = {
    ...
    "device_search": [
        ...
        ("get_search_scopes", "Available device-search scope options"),
        ("get_searchable_fields", "Searchable fields grouped by scope with type metadata"),
        ("list_searches_for_device", "Saved searches whose results contain a device"),
        ("run_saved_search", "Execute a saved search by UUID with paging + field projection"),
        ("create_saved_search", "Create a new saved device search"),
        ("update_saved_search", "Update an existing saved device search"),
        ("delete_saved_search", "Permanently delete a saved device search"),
        ("assign_policies_to_saved_search", "Bulk-assign policies to a saved-search result set"),
        ("refresh_saved_search_cache", "Force a re-cache of a saved search's results"),
    ],
    "policies": [
    ...
    "account": [
        ...
        ("list_users", "List users with name, email, and RBAC roles"),
        ("get_user", "Get a single user by numeric ID"),
        ("get_account", "Account detail (id, name, type, timestamps)"),
        ("list_account_rbac_roles", "Available RBAC roles in the account"),
        ("get_account_user", "Account-scoped user record (status, role, 2FA)"),
        ("list_zones_for_user", "Zones a given user belongs to"),
