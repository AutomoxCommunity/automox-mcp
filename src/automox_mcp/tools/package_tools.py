...
def register(server: FastMCP, *, read_only: bool = False, client: AutomoxClient) -> None:
    ...
    @server.tool(
        ...
        description=(
            ...
            "boolean. `severity` is one of critical/high/medium/low/"
            "no_known_cves or JSON null (live-verified); none/unknown exist per "
            "spec but were not observed live. JSON null means no severity assessment was "
    ...
    @server.tool(
        ...
        description=(
            ...
            "list_device_packages: critical/high/medium/low/no_known_cves or "
            "JSON null observed live, with none/unknown present in the spec enum "
            "but unobserved. See metadata.field_notes.severity. The endpoint "
            "returns a bare list with no total, so `returned_package_count` is "
            "the count of packages on THIS page only (NOT a fleet-wide org "
            "total); when metadata.pagination.has_more is true, increment `page` "
